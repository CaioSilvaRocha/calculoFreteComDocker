...
const conexaoBancoDados = mysql2.createConnection({
...
rotas.get("/obterHistoricos/:webToken", middlewareAutenticacaoWebToken, async (requisicao, resposta) => {
    ...
    conexaoBancoDados.query(comandoObtencaoHistoricos, [idContaUsuario], (erroObtencao, retornoObtencao) => {
...
rotas.post("/acessarConta", async (requisicao, resposta) => {
    ...
    conexaoBancoDados.query(comandoVerificacaoExistenciaConta, [nomeUsuario], async (erroVerificacao, retornoVerificacao) => {
...
rotas.post("/cadastrarConta", async (requisicao, resposta) => {
    ...
    conexaoBancoDados.query(comandoVerificacaoExistenciaCpf, [cpf], async (erroVerificacaoCpf, retornoVerificacaoCpf) => {
        ...
        conexaoBancoDados.query(comandoVerificacaoExistenciaEmail, [email], async (erroVerificacaoEmail, retornoVerificacaoEmail) => {
            ...
            conexaoBancoDados.query(comandoVerificacaoExistenciaNomeUsuario, [nomeUsuario], async (erroVerificacaoNomeUsuario, retornoVerificacaoNomeUsuario) => {
                ...
                if((retornoVerificacaoCpf.length === 0) && (retornoVerificacaoEmail.length === 0) && (retornoVerificacaoNomeUsuario.length === 0)){
                    ...
                    conexaoBancoDados.query(comandoCadastroConta, [nome, cpf, email, nomeUsuario, retornoHash], (erroCadastro, retornoCadastro) => {
...
rotas.post("/salvarCalculo/:webToken", middlewareAutenticacaoWebToken, async (requisicao, resposta) => {
    ...
    conexaoBancoDados.query(comandoVerificacaoExistenciaCalculo, [idContaUsuario, localOrigem, localDestino, distanciaLocalidades, consumoAutomovel, precoCombustivel, valorLucro, descricaoLucro], (erroVerificacao, retornoVerificacao) => {
        if(erroVerificacao) throw erroVerificacao;
        ...
        if(retornoVerificacao.length === 0){
            ...
            conexaoBancoDados.query(comandoArmazenamentoCalculo, [idContaUsuario, localOrigem, localDestino, distanciaLocalidades, consumoAutomovel, precoCombustivel, valorLucro, descricaoLucro, valorViagem], (erroArmazenamento, retornoArmazenamento) => {
